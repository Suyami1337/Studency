    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Участники проекта и настройка ролей.</p>
        </div>
        {tab === 'members' && (
          <button
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-2">
          {roles.map(r => (
            <button
              ...
              className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${activeRoleId === r.id ? 'bg-[#F0EDFF] text-[#6A55F8] font-medium' : 'text-gray-700 hover:bg-gray-50'}`}
            >
              <div className="flex items-center justify-between">
                <span>{r.label}</span>
                <span className="text-xs text-gray-400">{r.members_count}</span>
              </div>
