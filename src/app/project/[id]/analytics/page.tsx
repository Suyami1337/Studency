...
import { CustomerRow, FIRST_TOUCH_KIND_LABELS } from '@/lib/users/config'
...
function SourcesTab({ projectId }: { projectId: string }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ─── Аналитика трафика по UTM (атрибуция first-touch) ─── */}
      <SourcesAttributionAnalytics projectId={projectId} />
    </div>
  )
}

// ─── Аналитика трафика по UTM/first-touch (атрибуция) ───
type GroupKey = 'utm_source' | 'utm_campaign' | 'utm_medium' | 'first_touch_kind' | 'first_touch_source'
type Period = 'all' | '7d' | '30d' | '90d'

const ATTR_GROUP_LABELS: Record<GroupKey, string> = {
...
const ATTR_PERIOD_LABELS: Record<Period, string> = {
  all:  'Всё время',
  '7d': '7 дней',
  '30d': '30 дней',
  '90d': '90 дней',
...
function SourcesAttributionAnalytics({ projectId }: { projectId: string }) {
  const supabase = createClient()
  const [customers, setCustomers] = useState<CustomerRow[]>([])
  const [loading, setLoading] = useState(true)
  const [groupBy, setGroupBy] = useState<GroupKey>('first_touch_source')
  const [period, setPeriod] = useState<Period>('30d')
  const [sortKey, setSortKey] = useState<keyof AttrRow>('revenue')
  ...
  const rows = useMemo(() => {
    function getGroupKey(c: CustomerRow): { key: string; display: string } {
      ...
      const v = utm?.[groupBy]
      if (!v) return { key: '__none__', display: `— Без ${ATTR_GROUP_LABELS[groupBy]}` }
  ...
  const totals = useMemo(() => {
    ...
    for (const r of rows) {
      t.leads += r.leads; t.in_funnel += r.in_funnel; t.paid += r.paid; t.revenue += r.revenue
    }
    t.conversion = t.leads > 0 ? Math.round((t.paid / t.leads) * 1000) / 10 : 0
    return t
  }, [rows])

  function sortBy(k: keyof AttrRow) {
  ...
  return (
    <div className="space-y-4 mt-2 pt-6 border-t border-gray-100">
      <div>
        <h2 className="text-base font-semibold text-gray-900">Аналитика трафика</h2>
        <p className="text-xs text-gray-500 mt-0.5">Атрибуция по UTM-меткам и точкам входа: какие каналы реально приносят клиентов и выручку</p>
      </div>

      {/* Сводка */}
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        <AttrMetric label="Лидов" value={String(totals.leads)} />
        <AttrMetric label="В воронке" value={String(totals.in_funnel)} />
        <AttrMetric label="Клиентов" value={String(totals.paid)} highlight />
        <AttrMetric label="Выручка" value={formatMoney(totals.revenue)} highlight />
        <AttrMetric label="Конверсия" value={`${totals.conversion}%`} />
      </div>

      {/* Контролы */}
      <div className="flex items-center gap-3 flex-wrap">
        <div className="flex items-center gap-1.5 bg-white border border-gray-200 rounded-lg p-1">
          {(Object.keys(ATTR_PERIOD_LABELS) as Period[]).map(p => (
            <button
              key={p}
              onClick={() => setPeriod(p)}
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {(Object.keys(ATTR_GROUP_LABELS) as GroupKey[]).map(k => (
              <option key={k} value={k}>{ATTR_GROUP_LABELS[k]}</option>
            ))}
          </select>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-8 text-center">
          ...
          <p className="text-sm font-medium text-gray-700">Нет данных за период</p>
          <p className="text-xs text-gray-500 mt-1">Выберите другой период или дождитесь первых лидов</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr className="text-left text-xs text-gray-500">
                <th className="px-4 py-3 font-medium">{ATTR_GROUP_LABELS[groupBy]}</th>
                <AttrTh label="Лидов" k="leads" sortKey={sortKey} sortDir={sortDir} onClick={sortBy} />
                <AttrTh label="В воронке" k="in_funnel" sortKey={sortKey} sortDir={sortDir} onClick={sortBy} />
                <AttrTh label="Клиентов" k="paid" sortKey={sortKey} sortDir={sortDir} onClick={sortBy} />
                <AttrTh label="Выручка" k="revenue" sortKey={sortKey} sortDir={sortDir} onClick={sortBy} />
                <AttrTh label="Средний чек" k="avgCheck" sortKey={sortKey} sortDir={sortDir} onClick={sortBy} />
                <AttrTh label="Конверсия" k="conversion" sortKey={sortKey} sortDir={sortDir} onClick={sortBy} />
              </tr>
            </thead>
            <tbody>
              {rows.map(r => (
...
function AttrMetric({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
...
function AttrTh({
  ...
  label: string
  k: keyof AttrRow
  sortKey: keyof AttrRow
  sortDir: 'asc' | 'desc'
  onClick: (k: keyof AttrRow) => void
}) {
