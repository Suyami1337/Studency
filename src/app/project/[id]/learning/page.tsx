  ...
  return (
    <div className="space-y-5">
      ...
      <AiAssistantOverlay isOpen={aiOpen} onClose={() => setAiOpen(false)}
        ...
        initialMessages={[{ kind: 'ai' as const, text: `Привет! Я помогу с курсом "${course.name}". Сейчас ${modules.length} ${modules.length === 1 ? 'модуль' : 'модулей'}. Что делаем — структура уроков, тексты описаний, домашки, тарифы?` }]} />
    </div>
  )
}
